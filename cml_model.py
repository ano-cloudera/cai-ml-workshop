...
# PBJ runtimes (ml-runtime-pbj-*) do not auto-discover the model function by
# name the way legacy engines did — it must be registered with @cml_model.
# Without this the model loads ("Model ready" appears in the log) but every
# request fails with "failed to execute request" (a 400 with no useful
# message, since the request never actually reaches predict()).
try:
    import cml.models_v1 as models
    _cml_model = models.cml_model
except ImportError:                      # running outside CML, e.g. local tests
